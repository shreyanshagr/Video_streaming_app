...
function VideoUpload() {
  ...
  async function saveVideoToServer(video, videoMetaData) {
    ...
    try {
        ...
        formData.append("videoFile", selectedFile);  // Changed from 'file' to 'videoFile'

        const response = await axios.post(
            'http://localhost:8080/video',  // Ensure this matches your backend URL
            formData,
            {
                headers: {
        ...
        setMessage(`Error in uploading File: ${error.response?.data || error.message}`);
        setUploading(false);
